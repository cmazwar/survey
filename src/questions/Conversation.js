...
export default class Conversation extends Component {
    ...
    onSubmit() {
        ...
        if (submittedResponseText) {
            this.state.chat.addRobotChatResponse(submittedResponseText);
        }
    ...
    componentDidMount() {
        $('#cf-context').bind("DOMSubtreeModified", function (e) {

            // inputWrapper
            if($(".cf-button").length){
                $($(".cf-button")[0]).css(" pointer-events","none");
                $($(".cf-button")[6]).css("pointer-events","none");
    ...
    startConversation() {
        ...
        dispatcher.addEventListener(cf.ControlElementEvents.ON_LOADED, function(event){
           console.log("event")
           console.log(event)

        }, false);
        this.setState({
            chat: new cf.ConversationalForm({
                formEl: chatFormRef,
